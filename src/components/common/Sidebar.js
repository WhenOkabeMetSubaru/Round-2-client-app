...
function Sidebar (props)
{
    ...
    const checkUserPermissionsForRoute = ({ roles, permission_check }) =>
    {
        ...
        for (let i = 0; i < permission_check?.length; i++)
        {
            ...
            {
                if (roles[j]?.permissions?.includes('complete_access')) return true;
                ...
                <ul className="menu-list flex-grow-1 mt-3">
                    {
                        menuData.map((d, i) =>
                        {
                            ...
                            //     if (checkUserPermissionsForRoute({ roles: auth.getRoles(), permission_check: d?.permissions}) == false)
                            //     {
                            //         return <></>
                            //     }
                            ...
                            return (d?.permissions?.length > 0 && checkUserPermissionsForRoute({ roles: auth.getRoles(), permission_check: d?.permissions }) == false)?<></>: (<li key={ "shsdg" + i } className=" collapsed">
                                    ...
                                    d.children.length > 0 ?
                                        <ul className="sub-menu collapse has-children" id={ "menu-Pages" + i }>
                                            { d.children.map((data, ind) =>
                                            {
                                                ...
                                                {
                                                    if (checkUserPermissionsForRoute({ roles: auth.getRoles(), permission_check: data?.permissions }) == false)
                                                    {
                                                        return <></>
                                                    }
                                                }
